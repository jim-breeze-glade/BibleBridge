#!/usr/bin/env python3
"""
Simple TTS server runner that handles imports correctly
"""
...
def root():
    ...
    return jsonify({
        'service': 'BibleBridge TTS API',
        'version': '1.0.0',
        'endpoints': [
            'GET /api/pronunciation/health',
            'POST /api/pronunciation/audio',
            'GET /api/pronunciation/cache/stats',
            'POST /api/pronunciation/cache/clear'
        ]
    })

if __name__ == '__main__':
    print("🎙️  Starting BibleBridge TTS API server...")
    print("📡 Server will be available at: http://localhost:5001")
    print("🔍 Health check: http://localhost:5001/api/pronunciation/health")
    print("📋 API info: http://localhost:5001/")
    print("Press Ctrl+C to stop\n")
    
    # Check if Piper model is configured
    if not piper_tts_manager.piper_available:
        print("⚠️  Warning: Piper TTS not available. Please install piper-tts and configure a model.")
